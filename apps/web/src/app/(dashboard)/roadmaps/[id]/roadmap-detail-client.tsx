...
export default function RoadmapDetailClient({ id }: { id: string }) {
...
function CourseScreen({
  ...
  const handleShare = () => {
    ...
    const shareUrl = `${window.location.origin}/roadmaps/${roadmapId}${slugSuffix}`;
        ...
        <div className="flex items-center gap-4">
          ...
          {visibility === 'PUBLIC' && (
            <Button
              variant="outline"
              size="sm"
              className="h-8 rounded-lg border-white/10 text-muted-foreground hover:text-blue-400 hover:bg-blue-500/5 hover:border-blue-500/20 px-2.5 flex items-center gap-1.5 transition-colors duration-200"
              onClick={handleShare}
            >
              {copied ? (
                <>
